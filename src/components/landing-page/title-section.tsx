...
const TitleSection: React.FC<TitleSectionProps> = ({ title, subheading, pill }) => {
  return (
    <React.Fragment>
      <section className="flex flex-col justify-center items-start md:items-center gap-4">
        ...
        {subheading ? (
          ...
          <h2 className="text-left text-4xl sm:text-6xl sm:max-w-[850px] md:text-center font-semibold">{title}</h2>
        )}
      </section>
    </React.Fragment>
  );
